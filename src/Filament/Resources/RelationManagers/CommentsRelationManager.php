                ...
                Tables\Actions\CreateAction::make()->label('Add Comment')->modalHeading('Create Comment')
                    ->action(function (?Model $record, $data) {
                        ...
                        $comment = $issue->comments()->create([
                            'id' => $res['id'],
                            'body' => $data['body'],
                            'user_id' => \App\Filament\Resources\IssueResource\RelationManagers\auth()->id(),
                        ]);
                        Mail::to('joel@d3v.nz')->send(new \App\Mail\Issue\Comment($issue, $comment, \App\Filament\Resources\IssueResource\RelationManagers\auth()->user()));
                        return $comment;
                    }),
            ])
            ->actions([
